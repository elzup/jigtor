...
- REQ-OF09: タブは意味を持つ状態でのみ表示する。Edit は config があるとき (`S1`/`S3`)、
  Schema は config か schema があるとき (`S1`/`S2`/`S3`、config のみのときは A6/A7 の入口)、
  History は config 接続かつ保存履歴があるとき。`A6` 推論は `S1` の Schema タブから走り
  `S1 → S3` に遷移する。アクティブなタブが非表示になったら、表示中の先頭タブへ退避する。
