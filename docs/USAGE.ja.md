...
GitHub Release の配布ファイルをダウンロードして、静的ファイルとしてホストして開きます。
バックエンドは不要ですが、`file://` で `index.html` を直接開く使い方はサポートしません。
配布 zip は `dist/` ビルドを固めたもので、任意の静的ホスティングやローカル静的サーバーに
そのまま置けます。
...
4. 展開したフォルダを静的ファイルとして配信する
   - 例: GitHub Pages / Cloudflare Pages / S3 static website / nginx / 社内ファイルサーバー
   - ローカル確認なら、展開したフォルダで `python3 -m http.server 8000` を実行して
